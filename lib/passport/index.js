...
const debug = require('debug')('Javandi:PublisherPassportIndex');
  ...
  try {
    ...
    async.series({
      validateToken: function (callback) {
        if (!token) {
          return callback(new Boom.unauthorized('Token missing!', null));
        }
        return callback();
      },
      decodeUserToken: function (callback) {
        jwt.verify(token, APP_CONSTANTS.JWT.KEY, function (error, decodedToken) {
          if (error) {
            ...
            return callback(new Boom.unauthorized('Invalid token!', null));
          }
      ...
      findUserByDecodedToken: function (callback) {
        ...
        userModel.findOneByFilter({
          ...
          req.body.decoded_user = result.toObject();
          return callback();
